class SignPopup extends Popup{
    ...
    build(className){
        ...
        this.setForm()
        .then(this.sendFormData)
        .then(this.applySuccessEffect)
    }
    destroy(){
    ...
    setForm () {
        return fetch("view/form.html").then(response => response.text())
        .then(data => {
    ...
    sendFormData (e) {
        ...
        // var formData = "first_name=" + document.querySelector('input[name="first_name"]').value + "&last_name=" + document.querySelector('input[name="last_name"]').value + "&email=" + document.querySelector('input[type="email"]').value;
        return fetch (new Request('api.php', {
            method: 'POST',
            body: formData,
